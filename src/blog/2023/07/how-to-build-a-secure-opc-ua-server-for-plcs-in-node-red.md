...
:white_check_mark: Set up the PLC tags to be sent to the OPC Server
:white_check_mark: Read the PLC tags into Node-RED
...
Drag an `inject` node and `function` node on to the palette and wire them together.
...
msg.payload = conveyorData;
return msg;
```

You can see that each key/value pair in our dict corresponds directly with our PLC tag-names, and implicitly matches the datatype by how they’re initialized (ie `Robot_RTS` is a bool so we initialize it to `false`, and `Robot_Position` is a `float` so we initialize it to `0.0`).

Give the node an appropriate name, then hit done.

![function1-properties.png](./images/opc-ua-2/function1-properties.png)
Now that we’ve created a dict to store our PLC values in context memory, we need to actually move the PLC values coming from our `eth-ip in` node into it.

Drag another function node onto the palette and wire it to our `eth-ip in` node.
...
conveyorData.Robot_RTS = msg.payload.Robot_RTS;
conveyorData.Robot_Position = msg.payload.Robot_Position;
conveyorData.Conveyor_Running = msg.payload.Conveyor_Running;
conveyorData.Line4_State = msg.payload.Line4_State;
conveyorData.Line4_Fault = msg.payload.Line4_Fault;

flow.set('conveyorData', conveyorData);
return msg;
```

Give the node an appropriate name, hit done and deploy the flow.

![conveyor-context-function.png](./images/opc-ua-2/conveyor-context-function.png)
 

Our flow should now look like below.

![flow-with-plc-context.png](./images/opc-ua-2/flow-with-plc-context.png)
Let’s look at the flow context memory to confirm the data from our PLC is being written to our `conveyorData` object.
...
:white_check_mark: Set up the PLC tags to be sent to the OPC Server
:white_check_mark: Read the PLC tags into Node-RED
:white_check_mark: Copy the PLC tags into Node-RED context memory
...
:white_check_mark: Read the PLC tags into Node-RED
:white_check_mark: Copy the PLC tags into Node-RED context memory
:white_check_mark: Program the OPC Server address space
...
:white_check_mark: Copy the PLC tags into Node-RED context memory
:white_check_mark: Program the OPC Server address space
:white_check_mark: Encrypt the OPC Server with SSL
...
:white_check_mark: Program the OPC Server address space
:white_check_mark: Encrypt the OPC Server with SSL
:white_check_mark: Set up the OPC Client
