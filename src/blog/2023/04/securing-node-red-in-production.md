...
keywords: securing node-red, node-red security, node-red password, node-red https, node-red vpn, node-red authentication
tags:
    - posts
    - node-red
    - how-to
...
meta:
  howto:
    ...
    description: "Learn how to add password authentication, restrict LAN access, and encrypt Node-RED traffic using HTTPS or a VPN to protect your deployments."
    totalTime: "PT30M"
    tool:
      - "Node-RED"
      - "Nano (text editor)"
      - "Tailscale"
    steps:
      - name: "Protect Access from Your LAN"
        text: "Edit the Node-RED settings.js file to add a username and password using a hashed password generated by 'node-red admin hash-pw', then restart Node-RED to enforce login."
        url: "protecting-access-from-your-lan-to-node-red"
      - name: "Disable the Editor Interface"
        text: "Once flows are finalized, uncomment and set the editor disable option to true in settings.js so the flow editor is no longer accessible via the browser."
        url: "protecting-access-from-your-lan-to-node-red"
      - name: "Encrypt Traffic to Node-RED"
        text: "Install Tailscale VPN on both the Node-RED host and your client devices so all traffic between them is automatically encrypted without needing open firewall ports."
        url: "traffic-to-your-node-red-instance-is-unencrypted"
      - name: "Consider FlowFuse Cloud for Managed Security"
        text: "Use FlowFuse Cloud to get HTTPS, SSO, and role-based access control preconfigured, removing the need to manually manage certificates and authentication settings."
        url: "what-about-hosting-node-red-on-a-cloud-solution-such-as-flowfuse"
  faq:
    - question: "How do I add a password to Node-RED?"
      answer: "Edit the adminAuth section in settings.js, generate a hashed password with the 'node-red admin hash-pw' command, paste the hash into the settings file, and restart Node-RED."
...
<video autoplay loop muted playsinline aria-label="Using the username and password to login to Node-RED"><source src="/blog/2023/04/images/login.webm" type="video/webm"></source></video>
