...
meta:
  howto:
    name: "How to Establish a Defensible OPC UA Security Architecture"
    description: "Harden an OPC UA deployment by closing each known attack vector in order of impact: use Reverse Connect and network segmentation, enforce the trust list, force SignAndEncrypt with modern ciphers, layer user authentication and RBAC, automate certificate management with a GDS, keep auditing on, patch the stack, and test the configuration against policy."
    totalTime: "PT2H"
    tool:
      - "OPC UA server and client"
      - "Firewall / network segmentation"
      - "Global Discovery Server (GDS)"
      - "Company-specific Certificate Authority (CA)"
      - "Security testing tool"
    steps:
      - name: "Stop accepting inbound connections with Reverse Connect and segmentation"
        text: "Enable OPC UA Reverse Connect so the server dials out to the client with a ReverseHello message instead of accepting inbound connections, keeping the production-network firewall closed. Segment the control network from the enterprise network with firewalls between zones, and place a security gateway in the DMZ as the single aggregating access point."
        url: "stop-accepting-inbound-connections-at-all"
      - name: "Make the trust list actually enforce"
        text: "Turn off 'automatically accept all certificates' before production. Validate every presented certificate against the trusted, issuer, and rejected stores, promoting a certificate to trusted only by explicit administrator approval. Use a company-specific CA whose root you control rather than a commercial public CA. Copy each peer's certificate into the other party's trusted store, in both directions, because authentication is mutual."
        url: "make-the-trust-list-actually-enforce"
      - name: "Force SignAndEncrypt and remove deprecated ciphers"
        text: "Set the security mode to SignAndEncrypt for any endpoint carrying process data or accepting commands, and reserve None for localhost-only diagnostics. Remove the deprecated Basic128Rsa15 and Basic256 policies, use Basic256Sha256 as the enforced minimum, and prefer the modern Aes128_Sha256_RsaOaep or Aes256_Sha256_RsaPss policies."
        url: "force-signandencrypt-and-rip-the-deprecated-ciphers-out"
      - name: "Separate application authentication from user authentication"
        text: "Enforce both layers: application authentication proves which software connects, user authentication proves who operates the session. Use username/password only over a SignAndEncrypt endpoint, or a user X.509 certificate for machine-to-machine. Implement role-based access control from day one, assigning read, write, and browse separately per role under least privilege."
        url: "separate-who-the-software-is-from-who-the-operator-is"
      - name: "Manage certificates as infrastructure with a GDS"
        text: "Issue one X.509 v3 Application Instance Certificate per instance (2048-bit RSA minimum, 4096-bit for long-lived certs), with the SAN matching the application URI exactly. Track expiry, handle CRLs, and protect the private-key store. Automate issuance, renewal, and revocation across the fleet with a Global Discovery Server using pull or push management over standard PKI protocols."
        url: "manage-certificates-like-infrastructure-not-a-one-time-chore"
      - name: "Keep the audit trail on and watched"
        text: "Enable OPC UA audit events for security-relevant operations, then make sure a system collects and monitors them so a man-in-the-middle becomes an alert and a silent denial-of-service becomes an incident with a timeline."
        url: "keep-the-audit-trail-on-and-watched"
      - name: "Patch the stack and especially the gateways"
        text: "Track CVEs for your specific OPC UA stack and gateways, subscribe to OPC Foundation security bulletins and vendor advisories, and patch integration servers with the same urgency as the servers themselves. Apply client-side updates too, since a client trusts the server it connects to."
        url: "patch-the-stack-and-especially-the-gateways"
      - name: "Test what you think you built"
        text: "Verify the configuration matches policy: connect without a valid certificate to confirm the server rejects it, attempt unauthorized reads and writes to confirm RBAC denies them, and point a security testing tool at your endpoints to confirm the advertised security modes match the enforced ones."
        url: "test-what-you-think-you-built"
  faq:
