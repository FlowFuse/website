---
title: "MQTT, Sparkplug B, or Both? Choosing the Right Industrial Pub/Sub for Your Use Case"
  ...
  howto:
    ...
    steps:
      ...
      - name: "Decide whether you can tolerate losing a message"
        text: "Sparkplug B enforces QoS 0, fire and forget, with no delivery guarantee. If your data is regulated or safety-critical and every reading must be accounted for, use plain MQTT with a higher QoS level."
        url: "how-to-choose-between-mqtt-and-sparkplug-b"
...
| Delivery guarantee | Pick your QoS: 0, 1, or 2 | QoS 0 only ("fire and forget") |
...
Keeping it all consistent across multiple plants and vendors turns into a constant chore. This is the `plant1/line4/temp` problem from the intro, just multiplied by every team that ever touched the system. Nothing is wrong, exactly, and it all keeps running, but at some point nobody can open the broker and understand the whole thing anymore.
...
2. **Decide whether you can tolerate losing a message.** Sparkplug B enforces QoS 0, fire and forget, with no delivery guarantee. If your data is regulated or safety-critical and every reading must be accounted for, use plain MQTT with a higher QoS level.
...
The protocol is a means, not the goal. MQTT moves your data and asks almost nothing of you. Sparkplug B makes that data smaller, structured, and trustworthy at scale, but asks you to accept its conventions and its fire-and-forget delivery in return. My advice is the boring kind: don't trust benchmarks alone, test both in your real environment with your real data, and always plan for the bad day. What happens when the network drops or the broker crashes is where the difference between these two actually shows up.
