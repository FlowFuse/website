...
The best place to solve clock drift is at the operating system level. If the clock on an edge gateway, server, or data collection system stays accurate, every application running on that system benefits. If the clock is wrong, applications using that system time can create inaccurate timestamps.
...
If FlowFuse is already running on your edge devices, it is a convenient place to monitor whether the system clock used for generating timestamps remains accurate.
