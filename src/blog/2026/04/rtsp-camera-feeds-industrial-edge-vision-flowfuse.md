...
The pattern you built here is the foundation. From here, wire the raw frame output from the Assemble JPEG Frames node directly to an AI inference endpoint — skip the base64 step for that path, since inference endpoints expect raw buffers, not data URLs. Add a change node to attach metadata like camera ID, timestamp, and line number before the frame goes downstream. Connect multiple cameras by duplicating the flow and updating the RTSP URL and Secret in each ffmpeg node. The architecture stays the same. The flow grows with what the operation needs.
