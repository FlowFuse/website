...
This basic demo highlights how FlowFuse serves as an intelligent bridge to ensure accuracy between your production line and Odoo ERP. We're showcasing this by using a Raspberry Pi running the [FlowFuse device agent](/product/device-agent/) to communicate with a Siemens S7 PLC. A counter variable in the PLC, which increments every second to simulate ongoing production, is precisely read using the [S7 protocol](/blog/2025/01/integrating-siemens-s7-plcs-with-node-red-guide/) through FlowFuse. This accurate counter value is then automatically set as the quantity for a "table leg product" in Odoo, ensuring your inventory data is always up-to-date.
