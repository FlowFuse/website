...
|-----------------------------|--------------------------------------------------------------------------------|----------------------------------------------------------------|
| PLCs (Machine Controllers)  | Machine status, process values (temp, pressure, flow), control settings.       | Modbus, PROFINET, EtherNet/IP, OPC UA.                         |
| Sensors (Field Devices)     | Raw process values (temp, humidity, flow), environmental data, quality measurements. | Analog/Digital signals, IO-Link, HART, Wireless protocols, Modbus. |
| SCADA/HMI Screens           | Real-time operational status, operator inputs, recent historical trends.       | OPC Classic/UA, DNP3, Modbus TCP, EtherNet/IP.                 |
| Historians (Data Archives)  | Comprehensive historical process data, event logs, batch records.              | OPC HDA/UA, SQL, ODBC, MQTT.                                   |
| ERP (Enterprise Resource Planning) | Production orders, material consumption, labor tracking, quality specs, maintenance schedules. | SQL, HTTP APIs, OPC UA.                                        |
