...
description: Learn how to build an OEE Dashboard using FlowFuse.  
date: 2025-04-03
...
Before you begin building the OEE Dashboard with Node-RED Dashboard 2.0 and FlowFuse, make sure you have the following:
...
3. On deployment, it will create two tables in SQLite: `ProductionData` and `DowntimeData`.
...
Once the simulated data is generated and stored in SQLite, the next step is to configure settings for the flow and collect it for use in the OEE dashboard.
...
3. Drag the **Change node** onto the canvas, double-click it, and set the following properties:
...
Now that we have production and downtime data, we can calculate key OEE metrics: Availability, Performance, and Quality. Good units, defective units, total produced units, and target output are summed across all production lines. Total downtime is aggregated from downtime data to determine Availability. With these values, we can calculate OEE for the entire production system.
...
Now that we have all the necessary pieces, we can calculate the key metrics for OEE: Availability, Performance, and Quality. Once we have all three, we can proceed to calculate OEE.
...
Now to calculate the OEE of the last 30 days, we need the complete production and downtime data of the last 30 days, but the SQLite flow currently retrieves only the last 12 hours.
