...
By utilizing environment variables, this flow becomes reusable, allowing you copy and paste this to monitor multple isntances.
...
6. Connect the output of the Project In node to the input of the chart node.
...
To visualize CPU data for specific devices separately, configure the Project In node to "Listen for broadcast messages from" and select the desired instance name.

In the instance sending the CPU data, set the Project Out node to "Broadcast messages." This will send CPU data to all instances within your team, allowing the centralized instance to capture and display the information.
...
Before we begin, ensure you have following prepared:

- **node-red-contrib-dht-sensor**: install node via pallete manager, this node used to manage connection to a DHT11 or DHT22 sensor on a Raspberry Pi.
- **FlowFuse Device agent setup**: ensure you have setup and running flowfuse device agent on your raspberry pi and it is connected to platform.
...
1. Copy or download the above flow and import or upload it in the raspberyy node-red instance, make sure you have interfaced dht11 sensor correct with your raspberry pi for more ifnromation on it refer our guide on [Setting Node-RED on Raspberry Pi 4](/node-red/hardware/raspberry-pi-4/), which guide how to install setup device agent and then how to read temperature data form interfacing dht11 to ....

2. Deploy the flow

Once you deply the flow you will ssee the temperature data pritend on debug panel if you did everything correctly. now lets add prjec...
3. Drag the project in node onto the canvas, dobule click on it and set source to "Receive messages sent to this instance" and next enter the topic.
4. Replace the inject node with project in node.
5. Next you can add the change node to format the data recived by dht node.
6. Drag the project out node, dobule click on it and set the mode to "Return to project link call".

Once flow is ready deploy it. final flow should look like below:
...
Now, once you click the inject button, you will see the response in the debug panel that is read by the flow deployed on the Raspberry Pi.
...
FlowFuse project nodes streamline communication between Node-RED instances. By using these nodes, you can easily monitor performance and request data and more. This makes your workflows smoother and more efficient. Start using [FlowFuse](https://app.flowfuse.com/account/create) to enhance your Node-RED projects Today !