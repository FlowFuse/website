...
This is where AMQP comes in. AMQP addresses these challenges with features that MQTT doesn’t offer. In this guide, we’ll cover everything you need to get started with AMQP and Node-RED, including what AMQP is, how it compares to MQTT, and how to use it with Node-RED.
...
At the heart of AMQP is the **Message Broker**, which acts as the central hub for managing and routing messages. Producers, which are the systems or applications that create and send messages, send their data to the broker.

The broker uses **Exchanges** to determine how to route these messages. There are several types of exchanges:

- **Direct Exchange (point-to-point) :** Routes messages to specific queues based on an exact match with the routing key. For example, if a message has a routing key of "error", it will only go to queues that are set up to receive messages with the key "error".
...
- **Fanout Exchange:** Broadcasts messages to all queues bound to it, without considering the routing key. Every queue connected to this exchange receives a copy of the message.
...
|------------------------|-------------------------------------------|--------------------------------------------|----------------------------------------------|
| **Purpose**            | Good for reliable message delivery with complex routing. | Best for lightweight messaging, especially with IoT devices. | Designed for handling large amounts of data quickly. |
| **Message Delivery**   | Guarantees messages are delivered in the right order. | Tries to deliver messages, but sometimes the order or delivery isn't guaranteed. | Always delivers messages in the right order and ensures they are not lost. |
...
| **Message Handling**   | Can manage multiple queues and complex routing paths. | Simple; messages go directly to their destination or to multiple destinations. | Organizes data into topics for efficient handling of large streams of data. |
...
| **Fault Tolerance**    | High; messages are processed even if there are system problems. | Medium; message reliability depends on settings. | Very high; designed to handle failures and keep data safe. |
...
| **Typical Use Cases**  | Businesses needing dependable and detailed message handling. | IoT devices, mobile apps, and simple messaging scenarios. | Real-time data analysis, logging, and managing large-scale data. |
...
When it comes to choosing the right messaging protocol, it depends on your specific needs. If you need reliable, ordered message delivery with advanced routing capabilities, AMQP is a strong choice. It's ideal for applications where accurate message sequencing is crucial, even with network issues. MQTT, on the other hand, is best for lightweight and efficient communication, especially with IoT devices, where quick messaging and minimal setup are key. If you’re dealing with large volumes of data and require real-time processing, Kafka is the go-to option. It handles high-throughput data streams and complex data tasks like real-time analytics effectively. The right choice will depend on whether you prioritize reliability, speed, or managing large data volumes.

## Using AMQP with Node-RED

In this section, we’ll guide you through using AMQP with Node-RED. We'll cover setting up AMQP in Node-RED, configuring different exchange types, and integrating them into your flows. Throughout the guide, we will learn how to send and receive messages based on each of the exchange methods.
...
#### AMQP Broker 
   ...
   - **Port:** This is the network port through which the AMQP broker communicates. The default port for AMQP is 5672, but it might differ if configured otherwise.
   - **vhost:** Virtual hosts are used to segregate different environments or applications within the same broker instance. The default is `/`, but you might have specific virtual hosts for different use cases.
...
- **Exchange Configuration**
   ...
   - **Routing Key:** Used for directing messages to the correct queues based on the exchange type. It helps in specifying which queue should receive the message.
...
- **Broker:** Use the broker configuration details added with the AMQP Broker node to ensure that your incoming messages are received from the correct broker.
- **Prefetch:** Determines the number of messages to fetch from the broker in advance. This can help with performance by reducing the number of times the broker needs to send messages.
...
- **Exchange Configuration**
   - **Type:** Select the exchange type used to route incoming messages:
     ...
     - **Fanout**
     - **Headers**
   - **Exchange Name:** The exchange name where messages are routed from. This helps in directing incoming messages to the appropriate queue.
   - **Routing Key:** Specifies how to route messages from the exchange to the correct queue(s). This is essential for ensuring messages are received by the correct consumers.
...
- **Queue Info**
   ...
   - **Auto Delete:** Determines if the queue should be deleted automatically when no longer in use, helping to manage resources efficiently.
...
1. Drag two `inject` nodes on to the canvas. Configure the first `inject` node to send data with a `msg.routingKey` of `"zone1"` and the second with a `msg.routingKey` of `"zone2"`. set the payload for each of the inject node you want to send to zones.

2. Add an `amqp-out` node. Set the exchange to `"irrigation_control"`, which is where the commands will be sent.
...
we configured a Direct type exchange in Node-RED to route messages to specific queues based on the routing key. We demonstrated how to send and receive commands in a smart irrigation system, ensuring that messages for different zones are delivered correctly. This setup is useful when you need precise message delivery based on an exact match with the routing key.

## Topic Exchange

Scenario: You have a smart weather station that collects data from multiple sensors such as temperature, humidity, and air quality. You want to publish data and handle it based on the type of sensor and data such as all temperature sensors data or all humidity sensors data, you can use topic exchange which allows to use wild cards.
...
2. Connect each `amqp-in` node to a `debug` node to see the received sensor data.
...
Scenario: You have a smart home system with various components like lights, thermostats, and security cameras, and you want to broadcast status updates to all components simultaneously.

#### Sending Data using Fanout Exchange

1. Drag some inject nodes on to the canvas, set the payload for each this inject nodes will act as the components sending updates such lights, thermostat etc
2. Drag the mqtt-out node on to the canvas, Set the exchange type to `"Fanout"` and specify the exchange name as `"system_updates"`
3. Connect each inject node to the `amqp-out` node. This setup ensures that each status update payload is sent to the "system_updates" exchange, broadcasting it to all subscribed components.

#### Receiving Data from Fanout Exchange

1. Drag `amqp-in` nodes on to the canvas. Configure one to listen for messages from "weather_data"` exchange.

2. Connect `amqp-in` node to a `debug` node to see the received update from all your components data.
...
_Image showing the flow that uses the Fanout exchange type to send messages and receive messages._
...
Scenario: Imagine, you have different machines in a factory sending data about their operational status, such as whether they are running, idle, or experiencing an error. You want to route messages based on machine type, operational status, and priority level and has tow component in your monitoring system one that reciveds update from only CNC machine that has status as error and priority high and another that recives updates from all of the machines having the idle status and high priority. 
...
2. Drag the amqp-out node on to the canvas, Set the exchange type to `headers` and specify the exchange name as `system_update`
...
2. Connect each `amqp-in` node to a `debug` node to see the received updates for each of the component. 
