...
1. Drag the **Change** node onto the canvas, and set the `msg.payload` to `msg.payload.vehicles`.
...
2. Drag a **Function** node onto the canvas and paste the following code into it.
...
3. Connect **http-request** node's output to the **Change** node's input and the **Change** node's output to the **Function** node's input.
