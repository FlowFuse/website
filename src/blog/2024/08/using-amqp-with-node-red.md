...
- **Broker Configuration**
   - **Host:** The hostname or IP address of your AMQP broker.
   - **Port:** The port number used by the AMQP broker (default is 5672).
   - **vhost:** The virtual host on the broker (default is `/`).
   - **Use TLS:** Enable TLS/SSL if your broker requires secure connections.
   - **User:** Username for authentication with the broker, For the rabitmq broker it is by default guest.
   - **Password:** Password for authentication with the broker, For the rabitmq broker it is by default guest.

- **Reconnect Settings**
   - **Reconnect On Error:** Configure whether the node should attempt to reconnect automatically if an error occurs.

- **Exchange Configuration**
   - **Type:** Choose the exchange type:
     - **Topic:** Routes messages based on wildcard patterns in the routing key.
     - **Direct:** Routes messages to specific queues based on an exact match with the routing key.
     - **Fanout:** Broadcasts messages to all queues bound to the exchange.
     - **Headers:** Routes messages based on attributes in the message headers.
   - **Exchange Name:** The name of the exchange to which messages are published.
   - **Routing Key:** The key used to route messages to the correct queue(s).
   - **Durable:** Indicates if the exchange should be durable (survive broker restarts).

- **Message Properties**
   - **AMQP Properties:** Set additional properties for the messages, such as priority, expiration, or delivery mode, for more information refer to the [amqp docs](https://www.squaremobius.net/amqp.node/channel_api.html#channel_publish)
...
#### Sending Data to Direct Exchange
...
#### Receiving Data from Direct Exchange
...
![todo](./images/direct.gif){data-zoomable}
_todo_
...
![todo](./images/topic.gif){data-zoomable}
_todo_
{% renderFlow %}
...
![todo](./images/fanout.gif){data-zoomable}
_todo_
...
Scenario: You have a smart factory with machines and systems that generate various types of alerts (e.g., warnings, errors). You want to route these alerts based on their severity and type to the correct deparatment.

#### Sending Data to Headers Exchange

1. Drag two inject nodes on to the canvas. Configure the first `inject` node to send data with a `msg.headers` of `{ "severity": "error", "type": "system" }` and the second with a `msg.headers` of `{ "severity": "warning", "type": "system" }`. set the payload for each of the inject node you want to send to zones.

2. Drag the mqtt-out node on to the canvas, Set the exchange type to `headers` and specify the exchange name as `"system_update"`

#### Receiving Data from Direct Exchange

1. Drag two `amqp-in` nodes on to the canvas. Configure one to listen for messages with the `headers` of `{ "severity": "error", "type": "system" }` and the other with `{ "severity": "warning", "type": "system" }`. Both nodes should be set to the `system_update` exchange.

2. Connect each `amqp-in` node to a `debug` node to see the received updates for each of the department. 
