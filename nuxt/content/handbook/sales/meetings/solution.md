...
**[FlowFuse Application Guide](/docs/application-guide/)** and
established the [fundamentals](/docs/application-guide/foundations/) and the
[app delivery methods](/docs/application-guide/app-delivery-methods/). Solution
...
1. **App pattern** — decide the shape each app takes: a [hardware app](/docs/application-guide/app-delivery-methods/hardware-apps/) running on a device, or a [software app](/docs/application-guide/app-delivery-methods/software-apps/) running on the platform (three shapes each). Naming the pattern is what makes the PoV a test of *applications*, not of Node-RED.
2. **Data plane** — before deciding where things run, decide how data is handled in their environment. → [Data plane](/docs/application-guide/data-plane/)
3. **Architecture** — the defining decision is **where FlowFuse itself lives**: in the cloud, self-hosted on-prem, or air-gapped; serving the wider organisation from [IT](/docs/application-guide/architectures/it/), sitting in [OT](/docs/application-guide/architectures/ot/) near the equipment, or distributed across [IIoT](/docs/application-guide/architectures/iiot/) edge nodes ([overview](/docs/application-guide/architectures/)). Where FlowFuse lives sets the rest of the architecture — and is the same decision as which [FlowFuse Trial Environment](/handbook/sales/flowfuse-trial-environment/) the customer gets. This becomes the architecture stack on the PoV workbook's **Use Cases** tab.
