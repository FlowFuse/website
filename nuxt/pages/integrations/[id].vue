...
const hasExamples = computed(() => i.examples && i.examples.length > 0)
...
async function renderFlows() {
    if (rendered || !import.meta.client) return
    const containers = flowRenderers.value.filter(Boolean)
    if (!containers.length) return
    try {
        const FlowRenderer = await loadRenderer()
        i.examples.forEach((ex, idx) => {
            const container = flowRenderers.value[idx]
            if (!container || !ex.flow || container.dataset.rendered) return
            try {
                new FlowRenderer().renderFlows(ex.flow, {
                    container,
                    direction: 'LR',
                    gridLines: true,
                    zoom: true,
        ...
        // renderer load failed; leave placeholders
    }
}
...
function switchTab(name) {
    ...
    if (name === 'examples') {
        nextTick(() => setTimeout(renderFlows, 100))
    }
    if (import.meta.client) history.replaceState(null, '', `#${name}`)
}

onMounted(() => {
    if (window.location.hash.slice(1) === 'examples') switchTab('examples')
})
</script>
        ...
        <div class="flex flex-col md:flex-row gap-6 items-start w-full">
          <div class="flex-grow w-full">
            <div class="flex items-center gap-3 mb-4 flex-wrap">
              <h1 class="mb-0 text-3xl md:text-4xl font-bold break-words">{{ i.id }}</h1>
              <div v-if="i.ffCertified" class="ff-certified-badge flex items-center gap-2 bg-indigo-100 border-2 border-indigo-600 text-indigo-700 px-4 py-2 rounded-lg font-semibold text-sm whitespace-nowrap flex-shrink-0">
                <svg class="certified-icon w-5 h-5" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor"><path fill-rule="evenodd" clip-rule="evenodd" d="M8.603 3.799A4.49 4.49 0 0 1 12 2.25c1.357 0 2.573.6 3.397 1.549a4.49 4.49 0 0 1 3.498 1.307 4.491 4.491 0 0 1 1.307 3.497A4.49 4.49 0 0 1 21.75 12a4.49 4.49 0 0 1-1.549 3.397 4.491 4.491 0 0 1-1.307 3.497 4.491 4.491 0 0 1-3.497 1.307A4.49 4.49 0 0 1 12 21.75a4.49 4.49 0 0 1-3.397-1.549 4.49 4.49 0 0 1-3.498-1.306 4.491 4.491 0 0 1-1.307-3.498A4.49 4.49 0 0 1 2.25 12c0-1.357.6-2.573 1.549-3.397a4.49 4.49 0 0 1 1.307-3.497 4.49 4.49 0 0 1 3.497-1.307Zm7.007 6.387a.75.75 0 1 0-1.22-.872l-3.236 4.53L9.53 12.22a.75.75 0 0 0-1.06 1.06l2.25 2.25a.75.75 0 0 0 1.14-.094l3.75-5.25Z" /></svg>
                <span>FlowFuse Certified</span>
              </div>
            </div>
            <p class="text-lg text-gray-700 mb-4 w-full max-w-full break-words">{{ i.description }}</p>
            <div class="flex flex-wrap items-center gap-4 text-sm text-gray-600">
              <span class="flex items-center gap-1">
                <svg class="certified-icon w-5 h-5" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" d="M9.568 3H5.25A2.25 2.25 0 0 0 3 5.25v4.318c0 .597.237 1.17.659 1.591l9.581 9.581c.699.699 1.78.872 2.607.33a18.095 18.095 0 0 0 5.223-5.223c.542-.827.369-1.908-.33-2.607L11.16 3.66A2.25 2.25 0 0 0 9.568 3Z" /><path stroke-linecap="round" stroke-linejoin="round" d="M6 6h.008v.008H6V6Z" /></svg>
                <strong>Version:</strong> {{ i.version }}
              </span>
              <span class="flex items-center gap-1">
                ...
                <strong>Weekly Downloads:</strong> {{ formatNumber(i.downloadsWeek) }}
              </span>
              ...
              <span v-if="i.lastUpdated" class="flex items-center gap-1">
                ...
                <strong>Updated:</strong> {{ shortDate(i.lastUpdated) }}
              </span>
            </div>
          </div>
        </div>
    ...
    <div class="container m-auto max-w-6xl px-6 py-8">
      <div class="flex flex-col lg:flex-row gap-8 min-w-0">
        <div class="flex-grow min-w-0 overflow-hidden">
          <div v-if="i.ffCertified" class="mb-8 p-6 bg-indigo-50 border-l-4 border-indigo-600 rounded-r-lg overflow-hidden">
          ...
          <div v-if="hasExamples" class="border-b border-gray-300 mb-6">
            <nav class="flex gap-8" aria-label="Tabs">
              <button
                class="tab-button py-3 px-1 border-b-2 font-medium focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-indigo-600 focus-visible:ring-offset-2"
              ...
              >Examples ({{ i.examples.length }})</button>
            </nav>
          </div>
          ...
          <div class="prose max-w-none">
            <template v-if="hasExamples">
              ...
              <div v-show="activeTab === 'examples'" class="tab-content">
                ...
                <div v-for="(example, idx) in i.examples" :key="idx" class="bg-gray-50 border border-gray-200 rounded-lg p-6 mb-6">
                  ...
                  <div v-if="example.flow" class="bg-white border border-gray-300 rounded overflow-hidden mb-4">
                    <div :ref="(el) => (flowRenderers[idx] = el)" style="height: 400px;" class="flow-renderer-container" />
                  </div>
                  <p v-else class="text-gray-500 mb-4">Flow content not available</p>
                </div>
              </div>
            </template>
            <div v-else v-html="i.readmeHtml" />
          </div>
        </div>
          ...
          <div class="space-y-6">
            <div class="bg-white border border-gray-200 rounded-lg p-6 shadow-sm overflow-hidden">
              <h3 class="text-lg font-bold mb-3 flex items-center gap-2">
                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-5 h-5"><path stroke-linecap="round" stroke-linejoin="round" d="M17.25 6.75 22.5 12l-5.25 5.25m-10.5 0L1.5 12l5.25-5.25m7.5-3-4.5 16.5" /></svg>
                Installation
              </h3>
              ...
              <div class="bg-gray-900 text-gray-100 p-3 rounded font-mono text-sm overflow-x-auto">
                <code class="!bg-transparent !border-0 !p-0 text-gray-100 break-all whitespace-pre-wrap">npm install {{ i.id }}</code>
              </div>
            </div>
            ...
            <div class="bg-white border border-gray-200 rounded-lg p-6 shadow-sm overflow-hidden">
              ...
              <div class="space-y-3 text-sm">
                <div class="flex justify-between items-center pb-2 border-b">
                  ...
                  <span class="font-semibold">{{ i.version }}</span>
                </div>
                <div class="flex justify-between items-center pb-2 border-b">
                  <span class="text-gray-600">Weekly Downloads</span>
                  <span class="font-semibold">{{ formatNumber(i.downloadsWeek) }}</span>
                </div>
                <div v-if="i.created" class="flex justify-between items-center pb-2 border-b">
                  <span class="text-gray-600">First Published</span>
                  <span class="font-semibold">{{ shortDate(i.created) }}</span>
                </div>
                <div v-if="i.lastUpdated" class="flex justify-between items-center">
                  <span class="text-gray-600">Last Updated</span>
                  <span class="font-semibold">{{ shortDate(i.lastUpdated) }}</span>
                </div>
              </div>
            </div>

            <div v-if="i.githubOwner && i.githubRepo" class="bg-white border border-gray-200 rounded-lg p-6 shadow-sm overflow-hidden">
              <h3 class="text-lg font-bold mb-3">GitHub Stats</h3>
              <div class="space-y-2">
                <a :href="`https://github.com/${i.githubOwner}/${i.githubRepo}/issues`" target="_blank" rel="noopener noreferrer" class="flex items-center justify-between p-2 hover:bg-gray-50 rounded transition-colors">
                  ...
                  <img :src="`https://img.shields.io/github/issues/${i.githubOwner}/${i.githubRepo}.svg?style=flat-square`" alt="GitHub Issues" />
                </a>
                <a :href="`https://github.com/${i.githubOwner}/${i.githubRepo}/stargazers`" target="_blank" rel="noopener noreferrer" class="flex items-center justify-between p-2 hover:bg-gray-50 rounded transition-colors">
                  <span class="text-sm text-gray-600 flex items-center gap-2">
                  ...
                  <img :src="`https://img.shields.io/github/stars/${i.githubOwner}/${i.githubRepo}.svg?style=flat-square`" alt="GitHub Stars" />
                </a>
                <a :href="`https://github.com/${i.githubOwner}/${i.githubRepo}/network/members`" target="_blank" rel="noopener noreferrer" class="flex items-center justify-between p-2 hover:bg-gray-50 rounded transition-colors">
                  <span class="text-sm text-gray-600 flex items-center gap-2">
                  ...
                  <img :src="`https://img.shields.io/github/forks/${i.githubOwner}/${i.githubRepo}.svg?style=flat-square`" alt="GitHub Forks" />
                </a>
              </div>
            </div>
            ...
            <div class="bg-gray-50 border border-gray-200 rounded-lg p-6 overflow-hidden">
              ...
              <div class="space-y-2 text-sm">
                <a :href="`https://www.npmjs.com/package/${i.id}`" target="_blank" rel="noopener noreferrer" class="flex items-center gap-2 text-gray-700 hover:text-indigo-600 transition-colors">
            ...
            <div class="bg-gradient-to-br from-indigo-50 to-blue-50 border border-indigo-200 rounded-lg p-6 overflow-hidden">
              ...
              <a href="/contact-us/" class="inline-flex items-center justify-center w-full gap-2 bg-indigo-600 hover:bg-indigo-700 text-white px-4 py-2 rounded-lg font-semibold transition-colors duration-200 text-sm hover:no-underline">
                CONTACT US
                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-4 h-4"><path stroke-linecap="round" stroke-linejoin="round" d="M13.5 4.5 21 12m0 0-7.5 7.5M21 12H3" /></svg>
              </a>
            </div>
          </div>
        </div>
