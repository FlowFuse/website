...
// Ported from src/landing/plc.njk (11ty), which this replaces. Same page, same copy,
...
const FAQ = [
    ...
    },
    {
        ...
        answer: "Standardize on an integration layer that speaks open protocols instead of committing to one vendor's ecosystem. FlowFuse connects to PLCs and SCADA using OPC UA, MQTT, and Modbus, and pushes structured data to any MES, ERP, or database over REST or SQL. Because the logic lives in portable, open-source Node-RED flows rather than a proprietary gateway configuration, you can swap PLC brands, MES vendors, or cloud providers later without rebuilding the integration."
    },
    {
        question: "Why does PLC communication have latency, and how do I fix it?",
        answer: "Latency almost never comes from the protocol itself \u2014 it comes from polling every tag at the same rate, timeouts set from guesswork instead of measurement, too many requests competing for the controller's scan budget, and single-write telemetry instead of buffered batches. Tiering poll rates, measuring timeouts, and sampling at the source fixes most PLC communication latency without touching the network. See our full guide on [fixing PLC communication latency](/blog/2026/06/plc-communication-latency-causes-and-fixes/) for a step-by-step walkthrough."
    },
    {
        question: "Does FlowFuse work on-premises or in the cloud?",
        answer: "Both. FlowFuse runs on FlowFuse Cloud (fully managed, no infrastructure to maintain), self-managed on your own servers or private cloud, or directly on edge devices running the FlowFuse Device Agent. For PLC connectivity, many teams deploy the Device Agent on an industrial PC close to the machines, so data never has to leave the plant unless you choose to send it."
    }
]

const META_DESCRIPTION = 'Send PLC data to the cloud, SCADA, and MES without a gateway or vendor lock-in. Connect Siemens, Allen-Bradley, Omron & more via MQTT, OPC UA & Modbus with FlowFuse.'

useSeoMeta({
    title: 'PLC Integration Platform: Connect to MQTT & SCADA',
    ...
    ogImage: 'https://flowfuse.com/landing/images/plc-landing-image.png',
    ogUrl: 'https://flowfuse.com/landing/plc/',
...
<template>
  <div class="nohero w-full">
              ...
              <div class="md:flex md:my-16 items-center md:flex-row md:justify-between container mx-auto max-md:text-center md:max-w-screen-lg gap-8 items-stretch">
                  ...
                  <div class="md:w-2/5 flex-grow relative">
                      <div class="ff-image-rounded w-full">
                          <img src="/landing/images/plc-landing-image.png" alt="PLC connected to cloud, MQTT, and dashboards via FlowFuse" width="496" class="w-full h-auto">
                      </div>
                  </div>
              </div>
          </div>
      ...
      <!-- ============================================================
           PLC TO CLOUD / SCADA / MES — NO GATEWAY, NO VENDOR LOCK-IN
      ============================================================ -->
      <div class="w-full py-16 px-6">
          <div class="max-w-screen-lg mx-auto">
              <h2 class="mb-6">Send <span class="text-indigo-600">PLC Data to the Cloud</span>, Without a Gateway or Vendor Lock-In</h2>
              ...
              <div class="grid md:grid-cols-3 gap-8">
                  <div>
                      <h4 class="mt-0 mb-2 text-gray-800">PLC + SCADA, one integration</h4>
                      <p class="font-light text-gray-600">Pull tag data straight from the controller and poll your existing SCADA historian or OPC server in the same flow — one platform covers both layers instead of two separate integrations.</p>
                  </div>
                  <div>
                      <h4 class="mt-0 mb-2 text-gray-800">MES and ERP, without rebuilding per site</h4>
                      <p class="font-light text-gray-600">Route production counts, downtime, and quality data from the PLC into SAP, Oracle, or any MES over REST or SQL. Build the flow once and deploy it to every plant.</p>
                  </div>
                  <div>
                      <h4 class="mt-0 mb-2 text-gray-800">No vendor lock-in</h4>
                      <p class="font-light text-gray-600">Integration logic lives in open-source Node-RED flows, not a proprietary gateway config. Swap PLC brands, MES vendors, or cloud providers later without starting over.</p>
                  </div>
              </div>
              <div class="mt-10">
                  <CtaBookDemo variant="ghost" position="mid-page" icon="i-lucide-arrow-right" />
              </div>
          </div>
      </div>
          ...
          <div class="max-w-screen-lg mx-auto">
              ...
              <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6">
                  <div v-for="protocol in PROTOCOLS" :key="protocol.title" class="bg-white rounded-lg border border-gray-100 p-6 drop-shadow-sm flex flex-col">
                      ...
                      <NuxtLink v-for="link in protocol.links" :key="link.url" :to="link.url" class="text-indigo-600 text-sm font-medium mt-2 hover:underline">{{ link.text }}</NuxtLink>
                  </div>
              </div>
          </div>
      </div>
