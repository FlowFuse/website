...
<template>
  <div class="ff-utility-bar w-full bg-indigo-700 text-indigo-100 text-sm px-6">
    <div class="mx-auto max-screen-none lg:max-w-screen-xl 2xl:max-w-[1920px] flex items-center justify-between gap-4 py-3">
      ...
      <CtaSignIn variant="text" position="utility-bar-mobile" class="md:hidden shrink-0 text-indigo-100 hover:text-white no-underline hover:no-underline font-medium" />
      <!-- Quick links (right) -->
      <nav class="hidden md:flex items-center gap-5 shrink-0" aria-label="Quick links">
        ...
        <CtaSignIn variant="text" position="utility-bar" class="text-indigo-100 hover:text-white no-underline hover:no-underline font-medium" />
      </nav>
    </div>
  </div>
</template>
