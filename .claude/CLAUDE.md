...
One Nuxt application serves the whole site. It was generated by Eleventy (11ty) until
...
- **Framework**: Nuxt 4 (`nuxt/`) with `@nuxt/content` v3 for content-driven pages
...
pairs are gitignored: never edit them, a sync overwrites them.
...
npm run build:nuxt     # full build (Netlify production runs build:nuxt:skip-images, then index:algolia)
```
...
Tag options are defined in `nuxt/data/blogTags.json`. Future-dated posts are excluded from collections until their date arrives, in production builds only; deploy previews and dev show them.
...
sibling `../blueprint-library` checkout, then a clone of `BLUEPRINTS_REF` (default `main`). The library is private, so the clone
