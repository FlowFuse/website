...
- **A component reused across several pages shouldn't disambiguate `position` by prefixing which page embeds it.** It's a natural instinct once the same CTA call site starts rendering on two different routes — but PostHog already tags every event with the page URL, so a page/surface prefix on `position` just duplicates that dimension instead of describing placement. `nuxt/components/content/AgentSetupTabs.vue` does this today (a `surface` prop, defaulting to `'ai'`, prefixes `position` so the `/ai/` and `/docs/...third-party-agents/` copies of the same tabs are separable) — a known, deliberate-at-the-time exception, not yet reconciled with this convention. Don't copy the pattern into new shared components; if two instances of the same component need separable analytics, that's what the page URL already gives you for free.
